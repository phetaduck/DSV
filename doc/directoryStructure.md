...
* mainProjectFolder
    ...
    - **cmake** - contains cmake scripts that can be included with include()
        + DSVExtensions.cmake - reusable helpers for source/CMakeLists.txt
    ...
    - **packaging**
        ...
        + DSV.desktop - linux shortcut (app is then visible in launchers)
        + DSV.png - icon for linux shortcut
        + DSV.icon.in.rc - icon for windows description file (used in source/CMakeLists.txt)
        + DSV.ico - icon for windows shortcut (linked via DSV.icon.in.rc into the executable)
